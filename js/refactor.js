...
function fetchData(url) {
  fetch(url)
    .then((res) => res.json())
    .then((res) => createEmployeeArray(res))
    .then((res) => createCards(res.results))
    .catch((error) => console.log(error))
}

function createEmployeeArray(res) {
...
function createCard(employee, index) {
  ...
  for (let i = 0; i < cards.length; i++) {
    cards[i].addEventListener("click", (e) => {
      showModal(employees, cards[i].dataset.indexNumber);
    });
  }
};


function showModal(userArray, index) {

  createModal(userArray, index);
...
function createModal(userArray, index) {

  let employee = userArray[index];
